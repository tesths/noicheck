...
def test_process_submission_fetches_problem_and_generates_diagnosis(app, monkeypatch):
    ...
    monkeypatch.setattr("src.app.services.submission_pipeline.OpenJudgeProblemFetcher.fetch", fake_fetch)
    monkeypatch.setattr("src.app.services.submission_pipeline.DeepSeekDiagnosisService.diagnose", fake_diagnose)
...
def test_process_submission_stops_diagnosis_when_problem_fetch_fails(app, monkeypatch):
    def fake_fetch(self, url):
    ...
    monkeypatch.setattr("src.app.services.submission_pipeline.OpenJudgeProblemFetcher.fetch", fake_fetch)
    monkeypatch.setattr("src.app.services.submission_pipeline.DeepSeekDiagnosisService.diagnose", fail_if_diagnose_called)
